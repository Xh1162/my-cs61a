...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    opponentten = (opponent_score // 10) %10  #求对方十位数
    playerge= player_score % 10 #求我方个位数
    if opponentten == playerge:
        return 1
    else: return 3 * abs(opponentten-playerge)
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):       
    ...
    False
    """
    stvalue = strategy(0,0)
    def judge():
        for score in range(0,goal):
            for opponent_score in range(0,goal):
                if strategy(score,opponent_score) != stvalue:
                    return False
        return True
    return judge()
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    max_average_score = average_score(1,dice)
    max_num = 1
    for i in range(2,11):
        average_score_now = average_score(i,dice)
        if average_score_now > max_average_score:
            max_average_score = average_score_now
            max_num = i
    return max_num
    # END PROBLEM 9
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    if sus_points(score + take_turn(0,score,opponent_score)) - score >=threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
